...
3. **How can we combine the brevity of document languages with the power of programming languages?** Or put another way, what is the LaTeX / Markdown of the future? Raw <abbr>HTML</abbr> is not a pleasant format in which to hand-write documents, rich or otherwise. I want a document programming language that both (a) makes prose and common document elements easy to write, and (b) provides powerful and modern mechanisms for computation (i.e., not unhygienic macros à la TeX).
    ...
    - In <q><Ref id="document-calculus">A Core Calculus for Documents</Ref></q> (<abbr>POPL</abbr> 2024) I laid out a basic formalism for describing what computation means within a document.
