 ...
 */
registerComponent('look-controls2', {
  ...
  */
  ...
  updateOrientation: (function () {
    ...
    return function () {
      ...
      if (sceneEl.is('vr-mode') && sceneEl.hasWebXR && false) {
  ...
  onTouchStart: function (evt) {
    if (evt.touches.length !== 1 || (!isMobile() && !this.data.touchEnabled)) { return; }
    this.touchStart = {
  ...
  onTouchMove: function (evt) {
    ...
    if (!this.touchStarted || (!isMobile() && !this.data.touchEnabled)) { return; }

    deltaY = 2 * Math.PI * (evt.touches[0].pageX - this.touchStart.x) / canvas.clientWidth;
    deltaX = 2 * Math.PI * (evt.touches[0].pageY - this.touchStart.y) /
